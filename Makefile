...
# htdocs/index.html is generated by htdocs/Makefile (versioned asset URLs), on every make

.PHONY: htdocs/index.html install-dependencies

htdocs/index.html:
	@(cd htdocs; $(MAKE) --no-print-directory index.html)
...
# install the build / runtime dependencies (apt, libgpiod / libwebsockets from source if the
# distribution's version is too old), the optional parts according to the switches in Make.user

install-dependencies:
	@WOMO=$(WOMO) MOPEKA=$(MOPEKA) THETFORD=$(THETFORD) GARMIN=$(GARMIN) VDR=$(VDR) SRC_DIR=$(SRC_DIR) bash contrib/install-dependencies.sh
...
publish-deb:
	...
	echo 'put contrib/install-debian.sh' | sftp -i ~/.ssh/id_rsa2 p7583735@home26485763.1and1-data.host:$(TARGET)
	...
	echo 'chmod 755 install-debian.sh' | sftp -i ~/.ssh/id_rsa2 p7583735@home26485763.1and1-data.host:$(TARGET)
