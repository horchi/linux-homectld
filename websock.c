...
int cWebSock::init(int aPort, int aTimeout, const char* confDir, bool ssl)
{
   ...
   if (ssl)
   {
      ...
      info.options = LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT | LWS_SERVER_OPTION_VH_H2_HALF_CLOSED_LONG_POLL;
   }
...
int cWebSock::callbackHttp(lws* wsi, lws_callback_reasons reason, void* user, void* in, size_t len)
{
   ...
   // tell(eloAlways, "DEBUG: (%s) 'callbackHttp' got (%d) from (%p)", clientInfo.c_str(), reason, (void*)wsi);
   ...
   switch (reason)
   {
      ...
      case LWS_CALLBACK_HTTP:                    // 12
      {
         ...
         *sessionData = {};
         ...
         int res = serveFile(wsi, file);
         free(file);

         if (res < 0)
            return -1;

         tell(eloWebSock, "-> HTTP file '%s' served", url);

         break;
      }

      case LWS_CALLBACK_HTTP_BODY:     // 13 - incoming message body - POST
      {
         /*
      ...
      {
         /*
         tell(eloAlways, "HTTP: LWS_CALLBACK_HTTP_WRITEABLE");
         ...
         int res = lws_write(wsi, (unsigned char*)sessionData->buffer+sizeLwsPreFrame,
      ...
      case LWS_CALLBACK_HTTP_FILE_COMPLETION:     // 15
      {
         ...
         // this result in file load problems
      ...
      {
         // tell(eloDetail, "DEBUG: LWS_CALLBACK_HTTP_CONFIRM_UPGRADE (%p)", (void*)wsi);
         break;
      }

      case LWS_CALLBACK_CLOSED_HTTP:              // 5
      {
         // tell(eloAlways, "LWS_CALLBACK_CLOSED_HTTP %p\n", (void*)wsi);
         break;
      }
...
int cWebSock::serveFile(lws* wsi, const char* path)
{
   ...
   //   - html is excluded (no-store), it references the versioned asset names
   //   - files requested with a version tag (?v=<content hash>, generated by htdocs/genindex.sh,
   //     the icon font brings its own) never change under this URL -> a year, 'immutable'
   //   - all other files (images, ...) get tmoCacheMaxAge and a Last-Modified, the browser
   //     asks with If-Modified-Since afterwards and gets a 304 if unchanged

   bool isHtml {strcmp(mime, "text/html") == 0};
   char args[64] {};
   bool versioned {lws_hdr_copy_fragment(wsi, args, sizeof(args), WSI_TOKEN_HTTP_URI_ARGS, 0) > 0 && strncmp(args, "v=", 2) == 0};
   ...
   if (isHtml)
      ...
      sprintf(cacheControl, "max-age=%d, immutable", tmoCacheMaxAgeVersioned);
   else
      sprintf(cacheControl, "max-age=%d", tmoCacheMaxAge);

   struct stat st {};
   bool haveStat {stat(path, &st) == 0};

   // conditional request -> 304 without body if the file is unchanged

   if (haveStat && !isHtml && !versioned && lws_hdr_total_length(wsi, WSI_TOKEN_HTTP_IF_MODIFIED_SINCE) > 0)
   {
      char since[64] {};
      time_t tSince {0};

      if (lws_hdr_copy(wsi, since, sizeof(since), WSI_TOKEN_HTTP_IF_MODIFIED_SINCE) > 0
          && lws_http_date_parse_unix(since, strlen(since), &tSince) == 0 && st.st_mtime <= tSince)
      {
         if (lws_add_http_header_status(wsi, HTTP_STATUS_NOT_MODIFIED, &p, end)
             || lws_add_http_header_by_name(wsi, (const unsigned char*)"cache-control:",
                                            (const unsigned char*)cacheControl, (int)strlen(cacheControl), &p, end)
             || lws_add_http_header_content_length(wsi, 0, &p, end)
             || lws_finalize_http_header(wsi, &p, end))
         {
            tell(eloAlways, "HTTP: Failed to build 304 header for '%s'", path);
            return fail;
         }

         if (lws_write(wsi, start, lws_ptr_diff_size_t(p, start), LWS_WRITE_HTTP_HEADERS) < 0)
            return fail;

         tell(eloWebSock, "HTTP: '%s' not modified (304)", path);

         return lws_http_transaction_completed(wsi) ? fail : success;
      }
   }
   ...
   if (haveStat && !isHtml && !versioned)
   {
      char date[64] {};

      if (lws_http_date_render_from_unix(date, sizeof(date), &st.st_mtime) == 0
          && lws_add_http_header_by_name(wsi, (const unsigned char*)"last-modified:",
                                         (const unsigned char*)date, (int)strlen(date), &p, end) != 0)
      {
         tell(eloAlways, "HTTP: Failed to add last-modified header for '%s'", path);
         return fail;
      }
   }

   // a pre-compressed twin '<file>.gz' (created by 'make install') is served instead if the
   //   browser accepts gzip and the twin is not older than the original
   ...
   if (haveStat
       && lws_hdr_copy(wsi, encoding, sizeof(encoding), WSI_TOKEN_HTTP_ACCEPT_ENCODING) > 0 && strstr(encoding, "gzip")
       && stat(gzPath.c_str(), &stGz) == 0 && stGz.st_mtime >= st.st_mtime)
   {
      if (lws_add_http_header_by_token(wsi, WSI_TOKEN_HTTP_CONTENT_ENCODING, (const unsigned char*)"gzip", 4, &p, end)
          || lws_add_http_header_by_name(wsi, (const unsigned char*)"vary:", (const unsigned char*)"Accept-Encoding", 15, &p, end))
      {
         tell(eloAlways, "HTTP: Failed to add content-encoding header for '%s'", path);
         return fail;
      }
   ...
   if (res < 0)
   {
      tell(eloAlways, "HTTP: Failed to serve url '%s' (%d)", path, res);
      return fail;
   }
...
int cWebSock::callbackWs(lws* wsi, lws_callback_reasons reason, void* user, void* in, size_t len)
{
   ...
   switch (reason)
   {
      case LWS_CALLBACK_SERVER_WRITEABLE:                     // data to client
      {
         if (msgType == mtPing)
         {
            unsigned char buffer[sizeLwsFrame];
            tell(eloDebugWebSock, "DEBUG: Write 'PING' to '%s' (%p)", clientInfo.c_str(), (void*)wsi);
            lws_write(wsi, buffer + sizeLwsPreFrame, 0, LWS_WRITE_PING);
         }
         ...
         else if (msgType == mtData)
         {
            ...
            if (!clients[wsi].msgBufferDataPending() && !clients[wsi].messagesOut.empty())
            {
               ...
               size_t msgSize {clients[wsi].messagesOut.front().length()};
               size_t neededSize {sizeLwsFrame + msgSize};
               ...
               tell(eloWebSock, "=> (%zu) %.*s -> to '%s' (%p)", msgSize, (int)msgSize,
                    clients[wsi].msgBuffer+sizeLwsPreFrame, clientInfo.c_str(), (void*)wsi);
            }

            // on WSS connections bigger chunk sizes seems result in slow performance and locking of paket transmissions

            enum {chunkSizeMax = 10*1024};

            if (clients[wsi].msgBufferPayloadSize)
            {
               unsigned char* p {clients[wsi].msgBuffer + sizeLwsPreFrame + clients[wsi].msgBufferSendOffset};
               size_t pending {clients[wsi].msgBufferPayloadSize - clients[wsi].msgBufferSendOffset};
               size_t chunkSize {pending > chunkSizeMax ? chunkSizeMax : pending};
               int flags {lws_write_ws_flags(LWS_WRITE_TEXT, !clients[wsi].msgBufferSendOffset, pending <= chunkSizeMax)};

               int res {lws_write(wsi, p, chunkSize, (lws_write_protocol)flags)};

               if (res < 0 || (size_t)res < chunkSize)
               {
                  tell(eloAlways, "Error: lws_write chunk failed, wrote only %d instead of %zu bytes, client (%p) [%.*s]",
               ...
               if (clients[wsi].msgBufferPayloadSize || !clients[wsi].messagesOut.empty())
      ...
      case LWS_CALLBACK_RECEIVE:                           // 6 data from client
      {
      ...
      {
         tell(eloAlways, "Client '%s' connected (%p), ping time set to (%d)", clientInfo.c_str(), (void*)wsi, timeout);
         clients[wsi].wsi = wsi;
...
{
   tell(eloWebSock, "Client login '%s' (%p) [%s], now %zu clients in list", clientInfo, (void*)wsi, message, clients.size());
...
void cWebSock::atLogout(lws* wsi, const char* message, const char* clientInfo)
{
...

   {
      cMyMutexLock lock(&clientsMutex);
      clients.erase(wsi);
      tell(eloWebSock, "Client logout, now %zu clients in list", clients.size());
   }
}
